...
class InterviewerSerializer(serializers.ModelSerializer):
    ...
    )  # Изменили название на company_id
    ...
    def create(self, validated_data):
        ...
        if existing_interviewer:
            raise serializers.ValidationError(
                {"user_id": "Пользователь уже является интервьюером."}
            )

        if not company_id:
            raise serializers.ValidationError(
                {"company_id": "Поле 'company_id' обязательно для заполнения."}
            )

        # Если кандидата нет, создаем нового
        return Interviewer.objects.create(
            user=user, company=company_id, **validated_data
        )
