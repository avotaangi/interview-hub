...
class ResumeFilter(django_filters.FilterSet):
    min_salary = django_filters.NumberFilter(field_name="desired_salary", lookup_expr='gte', label="Минимальная зарплата")
    max_salary = django_filters.NumberFilter(field_name="desired_salary", lookup_expr='lte', label="Максимальная зарплата")
...
class ResumeSerializer(serializers.ModelSerializer):
    ...
    skills_data = serializers.PrimaryKeyRelatedField(queryset=Skill.objects.all(), many=True, write_only=True)
    job_experiences_data = serializers.PrimaryKeyRelatedField(queryset=JobExperience.objects.all(), many=True,
                                                              write_only=True)
    ...
    def create(self, validated_data):
        ...
        resume.job_experiences.set(job_experiences)  # Устанавливаем связи с опытом работы
    ...
    def update(self, instance, validated_data):
        ...
        instance.desired_position = validated_data.get('desired_position', instance.desired_position)
