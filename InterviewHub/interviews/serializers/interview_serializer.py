...
class InterviewSerializer(serializers.ModelSerializer):
    ...
    def validate_start_time(self, value):
        ...
        if value < now:
            raise serializers.ValidationError(
                "Время начала интервью должно быть в будущем."
            )
        return value
    ...
    def validate_hard_skills_rate(self, value):
        ...
        if value is not None and (value < 0 or value > 10):
            raise serializers.ValidationError(
                "Оценка хард-скиллов должна быть в диапазоне от 0 до 10."
            )
        return value
    ...
    def validate_soft_skills_rate(self, value):
        ...
        if value is not None and (value < 0 or value > 10):
            raise serializers.ValidationError(
                "Оценка софт-скиллов должна быть в диапазоне от 0 до 10."
            )
        return value
    ...
    def validate_recording_url(self, value):
        ...
        if value and not value.startswith("http"):
            raise serializers.ValidationError(
                "URL записи должен быть корректным и начинаться с http/https."
            )
        return value
