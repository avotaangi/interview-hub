...
class Interview(models.Model):
    ...
    hard_skills_rate = models.IntegerField(
        null=True, blank=True, verbose_name="Оценка хард скиллов"
    )
...
class InterviewTaskItem(models.Model):
    interview = models.ForeignKey(
        Interview, on_delete=models.CASCADE, verbose_name="Интервью"
    )
    ...
    class Meta:
        verbose_name = (
            "Элемент задания к интервью"  # Название таблицы в единственном числе
        )
        verbose_name_plural = (
            "Элементы задания к интервью"  # Название таблицы во множественном числе
        )
