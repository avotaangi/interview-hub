...
class CompanySelectionSerializer(serializers.ModelSerializer):
    ...
    interviewer_email = serializers.EmailField(
        source="interviewer.user.email", read_only=True
    )
    resume_candidate_email = serializers.EmailField(
        source="resume.candidate.user.email", read_only=True
    )

    # Добавляем поля для получения interviewer's id и resume's id
    interviewer_id = serializers.PrimaryKeyRelatedField(
        queryset=Interviewer.objects.all(), source="interviewer", write_only=True
    )
    ...
    def create(self, validated_data):
        ...
        company_selection = CompanySelection.objects.create(
            interviewer=interviewer, resume=resume, **validated_data
        )
